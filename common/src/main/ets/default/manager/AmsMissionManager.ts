...
class AmsMissionManager {
  ...
  async getOriginRecentMissionsList(): Promise<Array<missionManager.MissionInfo>> {
    try {
      ...
      Log.showError(TAG, `getOriginRecentMissionsList fail, err: ${JSON.stringify(err)}`);
    }
  }
  ...
  async clearMission(missionId: number): Promise<void> {
    ...
    try {
      ...
      Log.showError(TAG, `clearMission fail, err:${JSON.stringify(err)}`);
    }
  }
  ...
  async clearAllMissions(): Promise<void> {
    try {
      ...
      Log.showError(TAG, `clearAllMissions fail, err: ${JSON.stringify(err)}`);
    }
  }
  ...
  async getMissionSnapShot(missionId: number): Promise<SnapShotInfo> {
    ...
    try {
      ...
      Log.showError(TAG, `missionManager.getMissionSnapShot fail, missionId:${missionId} err: ${JSON.stringify(err)}`);
    }
    return snapShotInfo;
  }
