...
export default class PhoneSmartCanvas {
  ...
  private readonly DEFAULT_DISTANCE: number = 300;// mm
  private readonly DEFAULT_DISTANCE_COEFFICIENT: number = 1;
  ...
  static getInstance(info: DeviceScreenInfo): PhoneSmartCanvas {
    if (!globalThis.PhoneSmartCanvas) {
      globalThis.PhoneSmartCanvas = new PhoneSmartCanvas(info);
    }
    return globalThis.PhoneSmartCanvas;
  }
  ...
  private initIconSizeConfig(): void {
    this.calculatePPI();
    this.normalIconSize = this.calculateIconSize(this.DEFAULT_DISTANCE, this.DEFAULT_DISTANCE_COEFFICIENT);
  }
}